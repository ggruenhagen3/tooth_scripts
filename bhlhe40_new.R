...
# *** Even Though Everything Says SNPs, this is for all variants ***
library("Cairo")
library("TigR")
path_to_snp <- "C:/Users/miles/Downloads/d_tooth/data/whole_filter_rm_no_window_08_31_20.fst"
start <- 5594151
stop <- 8133628
...
snp_fst_data$COL[which(snp_fst_data$FST > 0.99)] = "black"
## snp_fst_data$COL[which(format(round(snp_fst_data$FST, 2), nsmall = 2) == "1.00")] <- "black"

# Plot the data
png_name <- "C:/Users/miles/Downloads/d_tooth/results/ts_ms/BHLHE40_Bicuspid_vs_Tricuspid_Fst.png"
Cairo(png_name, type="png", width = 13, height = 4, units = 'in', res=300)
par(fig=c(0,1,0,0.8))
plot(snp_fst_data$POS, snp_fst_data$FST, col=snp_fst_data$COL, xlab="Position on LG5", ylab=expression("Fst"), ylim=c(0,1), yaxt="n", main="",  xaxs="i")
...
title(main = expression("BHLHE40 Bicuspid vs Tricuspid Fst"), line = -0.4)
